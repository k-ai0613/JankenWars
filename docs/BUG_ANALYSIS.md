...
**28件中 27件を修正済み、1件が部分対応。** 修正内容は `npm test` の回帰テストで検証しています。

| 深刻度 | 件数 | 対応済 | 残 |
|-------|-----|-------|----|
| Critical | 2 | 2 | 0 |
| High | 7 | 6 | 1（H-7 は部分対応） |
| Medium | 11 | 11 | 0 |
| Low | 8 | 8 | 0 |

### 残っているもの

**H-7（依存脆弱性）— 48件 → 7件。** critical 2件と high 25件のうち 24件を解消しました
（`engine.io` と `socket.io-parser` の本番サーバ直撃分を含む）。
未使用だった `next-pwa` と `react-syntax-highlighter` は削除、`drizzle-orm` は修正版へ更新済みです。

残る7件はいずれもメジャーバージョンアップが必要で、アプリ全体の挙動に影響するため保留しています。

| パッケージ | 深刻度 | 必要な更新 | 備考 |
|-----------|-------|----------|------|
| `vite` | High | 5 → 8 | 開発サーバのみの脆弱性。本番ビルド成果物には影響しない |
| `react-router-dom` | Moderate | 6 → 7 | ルーティング API が変わるため全ページの確認が必要 |
| `drizzle-kit` ほか3件 | Moderate | — | 開発用ツールのみ。DB は現状未接続 |

いずれも `npm audit fix --force` で一括更新できますが、破壊的変更を伴うため判断を委ねています。

### 再発防止として入れたもの

- `shared/gameTypes.ts` / `shared/gameRules.ts` / `shared/events.ts` — 型・ルール・イベント名の単一情報源（分類 A・B・C・D の根本対策）
- `tsconfig.client.json` — クライアントを型チェック対象に追加（分類 A の検出機構）
- `eslint.config.js` — Lint の復旧（分類 F・未定義参照の検出）
- `tests/` — 全指摘の回帰テスト。`npm test` で shared ルール24件・i18n 3件・実サーバ12件を検証
- UI のルール文言を `WIN_LENGTH` から生成（分類 B の再発防止）

以下は監査時点の記録です。各項目の「対策」は実装済みの内容と一致します。
