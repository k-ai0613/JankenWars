...
const faqData: FAQItem[] = [
  // Gameplay Questions
  {
    ...
    answerEn: 'The goal is to be the first player to align 4 of your pieces in a row - vertically, horizontally, or diagonally - on the 6x6 game board. You place rock, paper, and scissors pieces strategically while using Janken (rock-paper-scissors) rules to capture opponent pieces.',
    answerJa: '6x6のゲームボード上で、縦・横・斜めのいずれかの方向に自分の駒を4つ並べることが目標です。グー・チョキ・パーの駒を戦略的に配置し、じゃんけんのルールで相手の駒を取りながら勝利を目指します。',
  },
  {
    category: 'gameplay',
    questionEn: 'How do Janken battles work?',
    questionJa: 'じゃんけんバトルはどのように行われますか？',
    answerEn: 'When you place a piece on a square occupied by an opponent\'s piece, a Janken battle occurs. Rock beats Scissors, Scissors beats Paper, and Paper beats Rock. You can only place your piece there if it wins; it then replaces the opponent\'s piece. A piece that would lose or tie cannot be placed there. The square becomes "locked" after a capture, and no one can place a piece there again.',
    answerJa: '相手の駒がある場所に自分の駒を置くと、じゃんけんバトルが発生します。グーはチョキに勝ち、チョキはパーに勝ち、パーはグーに勝ちます。置けるのは勝てる駒のときだけで、相手の駒を自分の駒に置き換えます。負け・あいこになる駒はそのマスに置けません。相手の駒を取ったマスは「ロック」され、以後どちらも駒を置けなくなります。',
  },
  {
    category: 'gameplay',
    questionEn: 'What is the Special Piece and how do I use it?',
    questionJa: '特殊駒とは何ですか？どのように使いますか？',
    answerEn: 'Each player has one Special Piece (marked with a star). It can only be placed on empty squares, cannot capture opponent pieces, and cannot be captured by opponents. It counts as part of your 4-in-a-row. Use it strategically to secure important positions that cannot be taken away.',
    answerJa: '各プレイヤーは1つの特殊駒（星マーク）を持っています。空いているマスにのみ配置でき、相手の駒を取ることも、相手に取られることもできません。4つ並べる列の一部としてカウントされます。奪われない重要な位置を確保するために戦略的に使いましょう。',
  },
  {
    category: 'gameplay',
    questionEn: 'What happens when a square is "locked"?',
    questionJa: 'マスが「ロック」されるとどうなりますか？',
    answerEn: 'When you capture a piece, that square becomes locked. No one can place a piece on a locked square again, so the capturing piece stays there for the rest of the game. This adds a strategic layer - capturing a key piece locks that position for good.',
    answerJa: '相手の駒を取ったマスはロックされます。ロックされたマスには以後どちらも駒を置けないため、取った駒は最後までそこに残ります。重要な位置の駒を取れば、その位置を確保できます。',
  },
  {
    category: 'gameplay',
    questionEn: 'How is my piece determined each turn?',
    questionJa: '毎ターンの駒はどのように決まりますか？',
    answerEn: 'In online games and when playing against the AI, you receive a random piece - Rock, Paper, or Scissors - at the start of each turn, and you place that piece (or your Special Piece instead). The AI opponent chooses its own piece. In local 2-player games, each player chooses which piece to place.',
    answerJa: 'オンライン対戦とAI対戦では、各ターンの開始時にグー・チョキ・パーのいずれかがランダムに配られ、その駒（または代わりに特殊駒）を置きます。AIは自分で駒を選びます。ローカルの2人対戦では、各プレイヤーが置く駒を自分で選べます。',
  },
  {
    category: 'gameplay',
    questionEn: 'What are the draw conditions?',
    questionJa: '引き分けの条件は何ですか？',
    answerEn: 'The game ends in a draw if: (1) The board is completely filled without either player achieving 4 in a row, or (2) Both players run out of pieces without achieving 4 in a row.',
    answerJa: '以下の場合、ゲームは引き分けとなります：(1) どちらのプレイヤーも4つ並べられないままボードが全て埋まった場合、(2) 4つ並べられないまま両プレイヤーが駒を使い切った場合。',
  },
  // Online Questions
  {
    category: 'online',
  ...
  },
  {
    ...
    answerEn: 'Currently, there is no time limit for moves. If a player disconnects during a game and does not reconnect within 15 seconds, or leaves the room during a game, that player loses. We recommend completing your moves within a reasonable time to ensure a good experience for both players.',
    answerJa: '現在、手番に時間制限はありません。ただし、対局中に切断して15秒以内に再接続できなかった場合や、対局中にルームを退出した場合は、そのプレイヤーの負けになります。両プレイヤーにとって良い体験を確保するため、適切な時間内に手を完了することをお勧めします。',
  },
  {
    category: 'online',
  ...
  },
  {
    ...
    answerEn: 'The AI analyzes the board state and evaluates potential moves based on factors like: controlling the center, creating winning threats, blocking opponent\'s winning moves, and strategic piece placement. It evaluates one move at a time; lower difficulty levels sometimes pick weaker moves on purpose, while Expert always plays the move it rates highest.',
    answerJa: 'AIはボードの状態を分析し、中央の制御、勝利の脅威の作成、相手の勝利手のブロック、戦略的な駒配置などの要素に基づいて、次の一手を評価します。難易度が低いほど評価の低い手をあえて選ぶことがあり、エキスパートは常に最も評価の高い手を選びます。',
  },
  {
    category: 'ai',
