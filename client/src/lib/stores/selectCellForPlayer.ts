import { PieceType, Player, Cell, Position } from '../types';
import { isValidMove, checkWin, checkDraw } from '../gameUtils';
import { useAudio } from './useAudio';
import { GamePhase, GameResult } from '../types';

// selectCellForPlayer関数の修正版 - バトルパターンなしの実装
export const selectCellForPlayer = (
  position: Position, 
  player: Player | string, 
  piece: PieceType, 
  board: any[], 
  player1Inventory: any, 
  player2Inventory: any, 
  get: () => any, 
  set: (state: any) => void
) => {
  // プレイヤー値を文字列として直接扱う
  ...
  if (targetCell.piece === PieceType.EMPTY) {
    ...
    // Trigger capture animation only - バトルパターン機能の削除
  ...
  // Update inventories - プレイヤーによって更新する状態を変える
  let newState;
  if (isPlayer1) {
    newState = {
      board: newBoard,
  ...
  // プレイヤーに応じたWin条件を適用
  const playerToCheck = isPlayer1 ? Player.PLAYER1 : Player.PLAYER2;
  const winResult = isPlayer1 ? GameResult.PLAYER1_WIN : GameResult.PLAYER2_WIN;
  const winMessage = isPlayer1 ? 'message.player1Win' : 'message.player2Win';
  
  // Check for win condition
  if (checkWin(newBoard, playerToCheck)) {
    set({
      ...newState,
      result: winResult,
  ...
  if (checkDraw(newBoard, player1Inventory, player2Inventory)) {
    set({
      ...newState,
  ...
  const nextPlayer = isPlayer1 ? Player.PLAYER2 : Player.PLAYER1;
  const nextPlayerMessage = isPlayer1 ? 'message.player2Turn' : 'message.player1Turn';
  ...
  // Get random piece for next player's turn
  setTimeout(() => {
